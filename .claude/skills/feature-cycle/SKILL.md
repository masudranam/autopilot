...
Contracts and migrations come before implementation. See `CLAUDE.md § Contracts`.

Write the tests as you go, one per acceptance criterion, each one able to fail if the behaviour
regresses. `CLAUDE.md § Testing` lists what is required and — just as important — what is
