...
describe('AuthRepository.rotateSession under concurrency', () => {
  ...
  it('a rotation that loses the row lock is refused, not granted a second successor (AC2)', async () => {
    ...
    // Long enough for the contender's pre-read to have run and its UPDATE to be parked
    // on the lock. It cannot proceed until the competitor commits, so this is a
    // lower bound on an event ordering rather than a guess at a duration.
